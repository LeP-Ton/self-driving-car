...
/**
 * 计算车辆从起点向前行驶的距离，作为舒适度相同或全员失败时的能力指标。
 */
 ...
 * 1. 纵向加速度反映加速和制动强度。
 * 2. 纵向冲击反映加速度是否突然变化。
 * 3. 横向加速度代理反映高速转弯带来的侧向感受。
 * 4. 转向突变反映左右方向是否快速反复切换。
 * 四项归一化后固定等权；只统计预热结束后仍未碰撞的帧。
 */
...
/** 将角度差约束到 -PI～PI，避免未来角度跨圈时产生虚假的巨大转向。 */
function normalizeAngle(angle) {
    return Math.atan2(Math.sin(angle), Math.cos(angle));
}

/** 将“越低越好”的平均成本转换为面板使用的 0～100 舒适度。 */
function comfortCostToScore(cost) {
    return 100 / (1 + Math.max(0, cost));
}

/**
 * 比较两组选择指标，返回正数表示左侧更适合作为亲本：
 * 1. 一存活一碰撞时，存活者优先。
 * 2. 都存活时，舒适成本更低者优先，距离用于裁决同成本。
 * 3. 都碰撞时，距离更远者优先，避免早撞车利用较少采样获利。
 */
 ...
 * 它让安全且更舒适的车辆更容易成为亲本，同时不给第一名绝对垄断权。
 */
